...
class ScheduleSlotItemSerializer(serializers.ModelSerializer):
    ...
    asset_uri = serializers.CharField(source='asset.uri', read_only=True)
    asset_mimetype = serializers.CharField(
        source='asset.mimetype', read_only=True,
    )
    asset_duration = serializers.IntegerField(
        source='asset.duration', read_only=True,
    )
    ...
    class Meta:
        ...
        read_only_fields = ['item_id', 'effective_duration']
...
class ScheduleSlotSerializer(serializers.ModelSerializer):
    items = ScheduleSlotItemSerializer(many=True, read_only=True)
    ...
    class Meta:
        ...
        read_only_fields = ['slot_id', 'items', 'is_currently_active']

    def get_is_currently_active(self, obj):
        return obj.is_currently_active()

    # ── days_of_week: accept list or JSON string, store as JSON string ──

    def to_internal_value(self, data):
        """Normalise days_of_week from list → JSON string before validation."""
    ...
    def to_representation(self, instance):
        """Deserialise days_of_week from JSON string → list for the API."""
    ...
    def validate(self, attrs):
        ...
        # Event slots cannot be default
        if slot_type == 'event' and is_default:
            raise serializers.ValidationError(
                'Event slots cannot be marked as default.'
            )

        # Enforce consistency: default type → is_default=True, event → no_loop=True
        ...
        if is_default or slot_type == 'default':
            # Default slot doesn't need time/overlap checks
            return attrs
        ...
        if slot_type == 'event':
            # Event slots: time_to is auto-computed from items.
            # On creation (no items yet) set time_to = time_from.
        ...
        # --- Time slot validation (original logic) ---
        time_to = attrs.get(
            'time_to',
        ...
        days_of_week_raw = attrs.get(
            'days_of_week',
            self.instance.days_of_week if self.instance else '[1,2,3,4,5,6,7]',
        )
        ...
        # Check overlap with existing TIME slots (event slots are excluded)
        existing = ScheduleSlot.objects.filter(
            is_default=False, slot_type='time',
        )
        if self.instance:
            existing = existing.exclude(slot_id=self.instance.slot_id)
        ...
        for slot in existing:
            ...
            if _time_ranges_overlap(
                ...
                raise serializers.ValidationError(
                    f'Time range overlaps with slot "{slot.name}" '
...
def _time_ranges_overlap(range_a, range_b):
    ...
    def expand(tf, tt):
        ...
        if sf < st:
            ...
            # Overnight: 22:00→06:00 becomes [(1320, 1440), (0, 360)]
            return [(sf, 1440), (0, st)]
